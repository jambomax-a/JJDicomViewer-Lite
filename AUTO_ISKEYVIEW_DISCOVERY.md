...
### 1. `drawRect:`メソッド（DCMView.m:3530-3531）
```objective-c
if( isKeyView == NO)
    [[self window] makeFirstResponder: self];
```

これは、描画時に自動的に`isKeyView`を設定する可能性があります。
...
`acceptsFirstMouse`が`YES`を返すということは、マウスがビューに入った時点で自動的に`becomeFirstResponder`が呼び出される可能性があります。

### 3. `acceptsFirstResponder`メソッド（DCMView.m:2662-2667）
```objective-c
...
`acceptsFirstResponder`が`YES`を返すということは、ビューが`FirstResponder`になることができることを意味します。

## 重要なポイント

### macOSの動作
- `acceptsFirstMouse`が`YES`を返すと、マウスがビューに入った時点で自動的に`becomeFirstResponder`が呼び出される可能性があります
- `drawRect:`で`isKeyView == NO`の場合に`makeFirstResponder`を呼び出すということは、描画時に自動的に`isKeyView`を設定する可能性があります
...
1. 描画が発生する
2. `drawRect:`が呼び出される
3. `isKeyView == NO`の場合、`makeFirstResponder`を呼び出す
4. `becomeFirstResponder`で`isKeyView = YES`が設定される
5. `becomeKeyWindow`で`sendSyncMessage: 0`が呼び出される
